  ...
  useEffect(() => {
    const fetchExerciseData = async () => {
      try {
        ...
        const statuses = statusRes?.metadata ?? [];

        const statusMap = new Map<number, ExerciseStatus>(
          statuses.map((s: { topic_id: number; exercise_status: ExerciseStatus }) => [
            s.topic_id,
            s.exercise_status
          ])
        );
        ...
        if (topics.length > 0) {
          const fetchedTopics = topics.map((item: TopicExerciseInfo) => ({
            ...item,
            id: item.topic_id,
            ...
            status: item.exercise_status ?? statusMap.get(item.topic_id) ?? 'not_started',
          }));
    ...
    >
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <Row gutter={[16, 16]} style={{ marginBottom: '32px' }}>
          <Col xs={12} sm={6}>
            ...
            >
              ...
              <div style={{ fontSize: '12px', color: '#8c8c8c' }}>Tổng bài tập</div>
          ...
          <Col xs={12} sm={6}>
            ...
            >
              ...
              />
              <div style={{ fontSize: '12px', color: '#8c8c8c' }}>Đã hoàn thành</div>
          ...
          <Col xs={12} sm={6}>
            ...
            >
              <ClockCircleOutlined
                style={{ fontSize: '24px', color: '#faad14', marginBottom: 8 }}
              />
              <div style={{ fontSize: '12px', color: '#8c8c8c' }}>Đang thực hiện</div>
          ...
          <Col xs={12} sm={6}>
            ...
            >
              <TrophyOutlined style={{ fontSize: '24px', color: '#722ed1', marginBottom: 8 }} />
              <div style={{ fontSize: '12px', color: '#8c8c8c' }}>Tỷ lệ hoàn thành</div>
              <div style={{ fontSize: '32px', fontWeight: 'bold', color: '#722ed1' }}>...</div>
            </Card>
          </Col>
        </Row>
        ...
        {paginatedExercises.length > 0 ? (
          <List
            grid={{ gutter: [24, 24], xs: 1, sm: 1, md: 2, lg: 3, xl: 3 }}
